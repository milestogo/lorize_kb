...
echo("Desktop Debug", "part", "X", "Y", "angle", "col", "row", "kicad switch location", "kicad LED", "kicad diode");
  ...
  difference() {
    ...
    // palm key trial.  In practice, the edge of the bezel probably keeps it
    // from being a good palm key on a PCB, you could make this a choice of 1u
    // or 1.5
    *for (x = [ -1, 1 ]) {
      for (y = [-pcby / 2 + rowstep / 2 + pcb_y_shift]) {
        translate([ x * (pcbx / 2 - colstep * (0.65)), y, 0 ]) dolongkey();
      }
    }
